...
package net.mcreator.plc.world.structure;
...
public class StructureOasis extends ElementsPolarcraft.ModElement {
	public StructureOasis(ElementsPolarcraft instance) {
		super(instance, 167);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldSouthpole.DIMID) {
			...
			isNetherType = WorldSouthpole.NETHER_TYPE;
		}
		if (dimID == WorldNorthpole.DIMID) {
			dimensionCriteria = true;
			isNetherType = WorldNorthpole.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 10000) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("plc", "oasisnbt"));
				if (template == null)
					return;
